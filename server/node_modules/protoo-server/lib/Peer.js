...
class Peer extends EnhancedEventEmitter
{
	...
	async request(method, data = undefined)
	{
		...
		this._logger.debug('[hgy] TX request() [method:%s, id:%s] data:%s', method, request.id, JSON.stringify(data));
	...
	_handleTransport()
	{
		...
		{
            this._logger.debug('[hgy] RX message %s', JSON.stringify(message));
			if (message.request)
