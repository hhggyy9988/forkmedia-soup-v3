  ...
  const uint8_t* data, size_t len, const struct sockaddr* addr, UdpSocket::onSendCallback* cb)
{
    ...
    {//scope
        const struct sockaddr* local = GetLocalAddress();
        if((local != NULL) && (addr != NULL) && (sent > 0)){
            char namebuf1[INET6_ADDRSTRLEN] = {0};
            char namebuf2[INET6_ADDRSTRLEN] = {0};
            if(GetLocalFamily() == AF_INET6){
                ...
                //MS_DEBUG_DEV("send:%s:%d --> %s:%d sent=%d", localip, localport, remoteip, remoteport, sent);
            } else {
		...
		{//scope
			...
			if((local != NULL) && (addr != NULL) && (len > 0)){
				...
				if(GetLocalFamily() == AF_INET6){
					...
					const char* localip  = inet_ntop(AF_INET6,	&v6l->sin6_addr, namebuf1, INET6_ADDRSTRLEN);
					const char* remoteip = inet_ntop(AF_INET6,	&v6r->sin6_addr, namebuf2, INET6_ADDRSTRLEN);
					//MS_DEBUG_DEV("send:%s:%d --> %s:%d sent2=%ld", localip, localport, remoteip, remoteport, len);
				} else {
					struct sockaddr_in*  v4l = (struct sockaddr_in*)local;
					struct sockaddr_in*  v4r = (struct sockaddr_in*)addr;
					int localport  = ntohs(v4l->sin_port);
  ...
  ssize_t nread, const uv_buf_t* buf, const struct sockaddr* addr, unsigned int flags)
{
	...
	if (nread > 0)
	{
		...
		{//scope
			const struct sockaddr* local = GetLocalAddress();
			if((local != NULL) && (addr != NULL) && (nread > 0)){
				char namebuf1[INET6_ADDRSTRLEN] = {0};
				char namebuf2[INET6_ADDRSTRLEN] = {0};
				if(GetLocalFamily() == AF_INET6){
					struct sockaddr_in6*  v6l = (struct sockaddr_in6*)local;
					...
					const char* localip  = inet_ntop(AF_INET6,	&v6l->sin6_addr, namebuf1, INET6_ADDRSTRLEN);
					const char* remoteip = inet_ntop(AF_INET6,	&v6r->sin6_addr, namebuf2, INET6_ADDRSTRLEN);
					//MS_DEBUG_DEV("OnUvRecv:%s:%d <-- %s:%d nread=%ld", localip, localport, remoteip, remoteport, nread);
				} else {
					struct sockaddr_in*  v4l = (struct sockaddr_in*)local;
					struct sockaddr_in*  v4r = (struct sockaddr_in*)addr;
					int localport  = ntohs(v4l->sin_port);
