...
namespace RTC
{
		...
		try
		{
			...
			for (auto& listenIp : listenIps)
			{
				if (enableUdp)
				{
					...
					// This may throw.
					//MS_DEBUG_DEV(" %p UdpSocket=%s", this, listenIp.ip.c_str());
	...
	void WebRtcTransport::HandleRequest(Channel::Request* request)
	{
		...
		switch (request->methodId)
		{
			...
			case Channel::Request::MethodId::TRANSPORT_RESTART_ICE:
			{
				...
				//MS_DEBUG_DEV("WebRtcTransport ICE usernameFragment and password changed [id:%s]", this->id.c_str());
	  ...
	  RTC::TransportTuple* tuple, const uint8_t* data, size_t len)
	{
		...
		{
			//MS_DEBUG_DEV("ignoring received packet of unknown type");
		}
	}
	  ...
	  const RTC::TransportTuple* tuple, const uint8_t* data, size_t len)
	{
		...
		{
			//MS_DEBUG_DEV("ignoring DTLS data coming from an invalid tuple");
			return;
		}
		...
		{
			//MS_DEBUG_DEV("Transport is not 'connecting' or 'connected', ignoring received DTLS data");
			return;
		}
	}
	  ...
	  RTC::TransportTuple* tuple, const uint8_t* data, size_t len)
	{
		...
		{
			//MS_DEBUG_DEV(" %p ignoring RTP packet while DTLS not connected", this);
			return;
		}
		...
		if (!this->srtpRecvSession)
		{
			//MS_DEBUG_DEV(" %p ignoring RTP packet due to non receiving SRTP session", this);
			return;
		}
		...
		if (!this->iceServer->IsValidTuple(tuple))
		{
			//MS_DEBUG_DEV(" %p ignoring RTP packet coming from an invalid tuple", this);
			return;
		}
		...
		if (!this->srtpRecvSession->DecryptSrtp(const_cast<uint8_t*>(data), &len))
		{
			...
			if (!packet)
			{
				//MS_DEBUG_DEV(" %p DecryptSrtp() failed due to an invalid RTP packet", this);
			}
			else
			{
				//MS_DEBUG_DEV(
				//  " %p DecryptSrtp() failed [ssrc:%" PRIu32 ", payloadType:%" PRIu8 ", seq:%" PRIu16 "]",
				//  this,
				//  packet->GetSsrc(),
		...
		if (!packet)
		{
			//MS_DEBUG_DEV(" %p received data is not a valid RTP packet", this);
			return;
		}

#if defined(HAS_DUMP_NETIO)
        {//scope
            const struct sockaddr* local = tuple->GetLocalAddress();
            const struct sockaddr* addr = tuple->GetRemoteAddress();
            if((local != NULL) && (addr != NULL) && (len > 0)){
                char namebuf1[INET6_ADDRSTRLEN] = {0};
                char namebuf2[INET6_ADDRSTRLEN] = {0};
                if(local->sa_family == AF_INET6){
                    struct sockaddr_in6*  v6l = (struct sockaddr_in6*)local;
                    struct sockaddr_in6*  v6r = (struct sockaddr_in6*)addr;
                    int localport  = ntohs(v6l->sin6_port);
                    int remoteport = ntohs(v6r->sin6_port);
                    const char* localip  = inet_ntop(AF_INET6,	&v6l->sin6_addr, namebuf1, INET6_ADDRSTRLEN);
                    const char* remoteip = inet_ntop(AF_INET6,	&v6r->sin6_addr, namebuf2, INET6_ADDRSTRLEN);
                    //MS_DEBUG_DEV(" %p %s:%d <-- %s:%d ssrc:%" PRIu32 ", payloadType:%" PRIu8 ", seq:%" PRIu16 " len=%ld",
                    //this, localip, localport, remoteip, remoteport,
                    //packet->GetSsrc(),   packet->GetPayloadType(), packet->GetSequenceNumber(), len);
                } else if(local->sa_family == AF_INET) {
                    struct sockaddr_in*  v4l = (struct sockaddr_in*)local;
                    struct sockaddr_in*  v4r = (struct sockaddr_in*)addr;
                    int localport  = ntohs(v4l->sin_port);
                    int remoteport = ntohs(v4r->sin_port);
                    const char* localip = inet_ntop(AF_INET,  &v4l->sin_addr, namebuf1, INET6_ADDRSTRLEN);
                    const char* remoteip = inet_ntop(AF_INET, &v4r->sin_addr, namebuf2, INET6_ADDRSTRLEN);
                    //MS_DEBUG_DEV("%p %s:%d <-- %s:%d ssrc:%" PRIu32 ", payloadType:%" PRIu8 ", seq:%" PRIu16 " len=%ld",
                    //this, localip, localport, remoteip, remoteport,
                    //packet->GetSsrc(), packet->GetPayloadType(), packet->GetSequenceNumber(), len);
                }
        ...
        //MS_DEBUG_DEV(" %p OK [ssrc:%" PRIu32 ", payloadType:%" PRIu8 ", seq:%" PRIu16 "]",
        //this,  packet->GetSsrc(), packet->GetPayloadType(), packet->GetSequenceNumber());
	  ...
	  RTC::UdpSocket* socket, const uint8_t* data, size_t len, const struct sockaddr* remoteAddr)
	{
		...
		if(remoteAddr->sa_family == AF_INET6){
			struct sockaddr_in6*  v6r = (struct sockaddr_in6*)remoteAddr;
			int remoteport = ntohs(v6r->sin6_port);
			const char* remoteip = inet_ntop(AF_INET6,	&v6r->sin6_addr, namebuf1, INET6_ADDRSTRLEN);
			//MS_DEBUG_DEV(" %p RX:%s:%d --> len=%ld", this, remoteip, remoteport, len);
		} else {
			struct sockaddr_in*   v4r = (struct sockaddr_in*)remoteAddr;
			int remoteport = ntohs(v4r->sin_port);
			const char* remoteip = inet_ntop(AF_INET, &v4r->sin_addr, namebuf1, INET6_ADDRSTRLEN);
			//MS_DEBUG_DEV(" %p RX:%s:%d --> len=%ld", this, remoteip, remoteport, len);
		}
		}
#endif
