...
namespace RTC
{
	  ...
	  : id(id), listener(listener)
	{
		...
		MS_DEBUG_DEV(" %p kind=%d ", this, (int)this->kind);
	...
	Producer::ReceiveRtpPacketResult Producer::ReceiveRtpPacket(RTC::RtpPacket* packet)
	{
		...
		if (!rtpStream)
		{
			MS_DEBUG_DEV("R1 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
		...
		if (packet->GetSsrc() == rtpStream->GetSsrc())
		{
			...
			if (!rtpStream->ReceivePacket(packet))
			{
                ...
                MS_DEBUG_DEV("R2 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
				return result;
			}
		}
		// RTX packet.
		else if (packet->GetSsrc() == rtpStream->GetRtxSsrc())
		{
			...
			if (!rtpStream->ReceiveRtxPacket(packet)){
				MS_DEBUG_DEV("R3 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
				return result;
			}
		}
		// Should not happen.
		...
		{
		    MS_DEBUG_DEV("R4 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
		...
		// If paused stop here.
		if (this->paused){
			MS_DEBUG_DEV("R5 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
			return result;
		}
		...
		if (!MangleRtpPacket(packet, rtpStream)){
			MS_DEBUG_DEV("R6 %p no stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
			return ReceiveRtpPacketResult::DISCARDED;
		}
        ...
        MS_DEBUG_DEV("R7 %p stream found for received packet [ssrc:%" PRIu32 "]", this, packet->GetSsrc());
		return result;
	}
	...
	RTC::RtpStreamRecv* Producer::GetRtpStream(RTC::RtpPacket* packet)
	{
		...
		for (size_t i{ 0 }; i < this->rtpParameters.encodings.size(); ++i)
		{
			...
			else if (isRtxPacket && encoding.hasRtx && encoding.rtx.ssrc == ssrc)
			{
				...
				MS_DEBUG_DEV(" %p add rtx [ssrc:%" PRIu32 "]", this, ssrc);
	  ...
	  RTC::RtpPacket* packet, const RTC::RtpCodecParameters& mediaCodec, size_t encodingIdx)
	{
		...
		MS_DEBUG_DEV(
		  " %p [encodingIdx:%zu, ssrc:%" PRIu32 ", rid:%s, payloadType:%" PRIu8 "]",
		  this,
		  encodingIdx,
		  ssrc,
		  encoding.rid.c_str(),
